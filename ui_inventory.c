...
static u8 getItemType(u16 item){
    ...
    switch(itemType){
        ...
        case ITEM_TYPE_NECTAR:
            itemType = ITEM_TYPE_EVOLUTION_ITEM;
        break;
...
static void PrintToWindow(u8 windowId, u8 colorIdx)
{
    ...
    for(i = 0; i < INVENTORY_MAX_ITEMS_SHOWN; i++){
        ...
        if(pocketId != TMHM_POCKET || itemId == ITEM_NONE){
            ...
            u8 moveType = gMovesInfo[ItemId_GetSecondaryId(itemId)].type;
        ...
        if(itemId != ITEM_NONE){
            ...
            if(pocketId != TMHM_POCKET){
                ...
                moveNum = ItemId_GetSecondaryId(itemId);
                CopyItemName(itemId, gStringVar1);
...
static void Task_ItemUseOnField(u8 taskId)
{
    ...
    if (ItemId_GetFieldFunc(itemId))
    {
        ...
        else if (CalculatePlayerPartyCount() == 0 && ItemId_GetType(itemId) == ITEM_USE_PARTY_MENU)
        {
            //There is no Pokemon
            sMenuDataPtr->currentSelectMode = INVENTORY_MESSAGE_CANT_USE_ITEM;
        }
...
static void Task_ItemUseOnBattle(u8 taskId)
{
    // Safety check
    u16 itemId = gSpecialVar_ItemId = getCurrentSelectedItemIdx();
    u16 type = ItemId_GetType(itemId);

    if (!ItemId_GetBattleUsage(itemId))
        return;
...
static void Task_MenuMain(u8 taskId)
{
    ...
    if (JOY_NEW(A_BUTTON)){
        ...
        else if(sMenuDataPtr->inventoryMode == INVENTORY_MODE_GIVEITEM){
