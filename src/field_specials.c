...
bool8 Script_CheckForMonAndMove(void)
{
    ...
    {
        u16 species = GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SPECIES);
        if (species == VarGet(VAR_TEMP_1) && MonKnowsMove(&gParties[B_TRAINER_PLAYER][i], VarGet(VAR_TEMP_2)))
        {
...
{
    CopyMon(&gSaveBlock1Ptr->stolenTrade, &gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], sizeof(struct Pokemon));
}
...
void CopyMonFromStolenTradeStorage(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        {
            CopyMon(&gParties[B_TRAINER_PLAYER][i], &gSaveBlock1Ptr->stolenTrade, sizeof(struct Pokemon));
...
void RandomlyBoostPartyMemberFriendship(void)
{
    ...
    for (u32 i = 0; i < PARTY_SIZE; i++)
    {
        pokemon = &gParties[B_TRAINER_PLAYER][party[i]];
