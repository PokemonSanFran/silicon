...
static void Task_ChangeScreen(u8 taskId)
{
    ...
    case SCREEN_CHANGE_ITEM_FROM_BAG:
        FreePokeStorageData();
        //GoToBagMenu(ITEMMENULOCATION_PCBOX, 0, CB2_ReturnToPokeStorage); //asdf
        Inventory_Init(CB2_ReturnToPokeStorage, INVENTORY_MODE_GIVE_ITEM);
...
static bool32 ShouldBoxmonSpriteBeTransparent(u32 boxId, u32 boxPosition)
{
    ...
    // End surpriseTrade
     && IsBoxMonExcluded(GetBoxedMonPtr(boxId, boxPosition)))
        return TRUE;
    // Start siliconMerge
    return (IsFaintedBoxMon(GetBoxedMonPtr(boxId,boxPosition)));
    //return FALSE;
    // End siliconMerge
}
