...
void SavePlayerPartyBattleFrontier(void)
{
    ...
    gSaveBlock1Ptr->playerPartyCount = gPartiesCount[B_TRAINER_PLAYER];

    for (i = 0; i < PARTY_SIZE; i++)
        gSaveBlock1Ptr->playerPartyBattleFrontier[i] = gParties[B_TRAINER_PLAYER][i];
}
...
void LoadPlayerPartyBattleFrontier(void)
{
    ...
    gPartiesCount[B_TRAINER_PLAYER]= gSaveBlock1Ptr->playerPartyCount;

    for (i = 0; i < PARTY_SIZE; i++)
        gParties[B_TRAINER_PLAYER][i] = gSaveBlock1Ptr->playerPartyBattleFrontier[i];
}
// End siliconMerge
