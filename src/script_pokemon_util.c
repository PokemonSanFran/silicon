...
void FaintPlayerParty(void)
{
    ...
    for (i = 0; i < gPartiesCount[B_TRAINER_PLAYER]; i++)
        FaintPokemon(&gParties[B_TRAINER_PLAYER][i]);
}
...
u32 ScriptGiveMonParameterized(u8 side, u8 slot, enum Species species, u8 level, enum Item item, enum PokeBall ball, u8 nature, u8 abilityNum, u8 gender, u16 *evs, u16 *ivs, enum Move *moves, enum ShinyMode shinyMode, bool8 gmaxFactor, enum Type teraType, u8 dmaxLevel)
// End siliconMerge
{
    struct Pokemon mon;
    u32 i;
