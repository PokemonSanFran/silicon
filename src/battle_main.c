...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            CreateMon(&party[i], HandleScaledSpecies(partyData[monIndex].species), HandleScaledLevel(partyData[monIndex].lvl,monsCount), 0, TRUE, personalityValue, otIdType, fixedOtId);
...
// End midBattleEvolution
        {
            u16 species = GetEvolutionTargetSpecies(&gPlayerParty[i], EVO_MODE_BATTLE_SPECIAL, i, NULL, &canStopEvo, DO_EVO);
...
// End midBattleEvolution
            ...
            if (species == SPECIES_NONE && (gLeveledUpInBattle & (1u << i)))
            {
                ...
                species = GetEvolutionTargetSpecies(&gPlayerParty[i], EVO_MODE_BATTLE_ONLY, gLeveledUpInBattle, NULL, &canStopEvo, DO_EVO);
            }
