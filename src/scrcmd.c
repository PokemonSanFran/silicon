...
bool8 ScrCmd_showmoneybox(struct ScriptContext *ctx)
{
    ...
    //DrawMoneyBox(GetMoney(&gSaveBlock1Ptr->money), x, y); // siliconMerge
    return FALSE;
}
...
bool8 ScrCmd_updatemoneybox(struct ScriptContext *ctx)
{
    ...
    //ChangeAmountInMoneyBox(GetMoney(&gSaveBlock1Ptr->money)); // siliconMerge
    return FALSE;
}
...
bool8 ScrCmd_bufferabilityname(struct ScriptContext *ctx)
{
    ...
    StringCopy(GetStringVar(stringVarIndex), gAbilitiesInfo[ability].name);
    return FALSE;
}
...
bool8 ScrCmd_buffermapname(struct ScriptContext *ctx)
{
    ...
    GetMapName(GetStringVar(stringVarIndex), mapSecId, 0);
    return FALSE;
}
// End siliconMerge
...
bool32 ScrCmd_ismonshiny(struct ScriptContext *ctx)
{
    ...
    gSpecialVar_Result = IsMonShiny(&gParties[B_TRAINER_PLAYER][partyIndex]);
    return FALSE;
}
...
bool8 ScrCmd_bufferspeciesheight(struct ScriptContext *ctx)
{
    ...
    ConvertIntToDecimalStringN(GetStringVar(stringVarIndex), height, STR_CONV_MODE_LEFT_ALIGN, CountDigits(height));
...
{
    u8 stringVarIndex = ScriptReadByte(ctx);
    FormatCurrentTimeForOutput(GetStringVar(stringVarIndex));
}
...
void ScrFunc_timeprintinggiven(struct ScriptContext *ctx)
{
    ...
    FormatGivenTimeForOutput(GetStringVar(stringVarIndex),hours,minutes);
}
// End timePrinting
