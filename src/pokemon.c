...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    if (field > MON_DATA_ENCRYPT_SEPARATOR)
    {
		...
		case MON_DATA_IS_FAINTED:
            retVal = GetSubstruct0(boxMon)->isFainted;
            break;
			// End siliconMerge
        case MON_DATA_MOVE1:
...
void SetBoxMonData(struct BoxPokemon *boxMon, s32 field, const void *dataArg)
{
    ...
    if (field > MON_DATA_ENCRYPT_SEPARATOR)
    {
		...
		// Start siliconMerge
        case MON_DATA_IS_FAINTED:
            SET8(GetSubstruct0(boxMon)->isFainted);
            break;
			// End siliconMerge
        case MON_DATA_MOVE1:
...
{
    s32 i;
    struct Pokemon *party = FlagGet(B_FLAG_SKY_BATTLE) ? gSaveBlock1Ptr->playerParty : gParties[B_TRAINER_PLAYER]; // flyEncounters

    for (i = 0; i < PARTY_SIZE; i++)
        // Start flyEncounters
        //if (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SPECIES) == SPECIES_NONE)
        if (GetMonData(party, MON_DATA_SPECIES) == SPECIES_NONE)
        // End flyEncounters
            return FALSE;
...
s32 MonGainEVs(struct Pokemon *mon, enum Species defeatedSpecies)
// End printEVs
{
    u8 evs[NUM_STATS];
    u16 evIncrease = 0;
...
void FaintBoxPokemon(struct BoxPokemon *boxMon)
{
    u32 data;
...
{
    species = SanitizeSpeciesId(species);
    if (P_CRIES_ENABLED == FALSE || gSpeciesInfo[species].cryId >= CRY_COUNT || gTestRunnerHeadless || AreCriesMuted())
        return CRY_NONE;
...
// Start pokedex
bool32 IsSpeciesFromAlola(enum Species species)
{
