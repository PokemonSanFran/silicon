    ...
    else
    {
         ...
         && !(gBattleStruct->battlerState[battler].notOnField))
        {
            ...
            if (IsOnPlayerSide(battler))
            {
                ...
                SetFaintedMonBit(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battler]]); //siliconMerge
            }
            else
            {
...
void UpdateStatsAfterLevelUp(u32 monId)
{
    ...
    if (battler != 0xFF)
    {
        CopyMonLevelAndBaseStatsToBattleMon(battler, &gParties[B_TRAINER_PLAYER][monId]);
...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            u32 monLvl = GetMonData(&gParties[B_TRAINER_PLAYER][*expMonId], MON_DATA_LEVEL); // printEVs
...
// End pointsMessage
                    //MonGainEVs(&gParties[B_TRAINER_PLAYER][*expMonId], gBattleMons[gBattlerFainted].species); // printEVs
                }
                gBattleScripting.getexpState++;
            }
        }
...
// Start trainerExpTests
                if (TESTING && gSiliconExpTestState.isExpTest && gBattleTurnCounter == 0)
                {
                    ...
                    {
                        expToReward = gSiliconExpTestState.backExp - GetMonData(&gParties[B_TRAINER_PLAYER][1], MON_DATA_EXP);
                        gSiliconExpTestState.useBackMon = FALSE;
                    }
                    else
                    {
                        expToReward = gSiliconExpTestState.expTestExp - GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_EXP);
...
// Start midBattleEvolution
            ...
            if (IsMidBattleEvolutionOn())
            {
                if ((gLeveledUpInBattle & (1u << *expMonId)) && !(gTriedEvolving & (1u << *expMonId)))
                {
                    ...
                    u32 species = GetEvolutionTargetSpecies(&gParties[B_TRAINER_PLAYER][*expMonId], EVO_MODE_BATTLE_ONLY, gLeveledUpInBattle, NULL, &canStopEvo, DO_EVO);
                    gTriedEvolving |= 1u << *expMonId;
                    if (species != SPECIES_NONE)
                    {
                        gMidBattleEvo = TRUE;
                        BtlController_EmitMidBattleEvo(gBattleStruct->expGetterBattlerId, B_COMM_TO_CONTROLLER, *expMonId, species);
                        MarkBattlerForControllerExec(gBattleStruct->expGetterBattlerId);
...
static void Cmd_switchinanim(void)
{
    ...
    MarkBattlerForControllerExec(battler);
// Start siliconMerge
    if(QuestMenu_GetSetQuestState(QUEST_ARTISANBALLS3,FLAG_GET_ACTIVE))
...
static void Cmd_trygivecaughtmonnick(void)
{
    ...
    case 3:
        if (gMain.callback2 == BattleMainCB2 && !gPaletteFade.active)
        {
            ...
            //gBattleCommunication[MULTIUSE_STATE]++;
            if (GetNicknameOption() == BATTLE_OPTION_NICKNAME_FORCED && !IsMonNicknamed(&gParties[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[gBattlerTarget]]))
            {
...
void BS_HandleExpTest(void)
{
    ...
    for (u32 i = 0; i < gPartiesCount[B_TRAINER_OPPONENT_A]; i++)
    {
        //  Revive all mons in the party
        if (gSiliconExpTestState.currentMon <= gSiliconExpTestState.numMons + 1)
            gParties[B_TRAINER_OPPONENT_A][i].hp = gParties[B_TRAINER_OPPONENT_A][i].maxHP;
    }
...
void BS_HandleExpTestFinish(void)
{
    ...
    gSiliconExpTestState.expTestExp = GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_EXP);
    gSiliconExpTestState.backExp = GetMonData(&gParties[B_TRAINER_PLAYER][1], MON_DATA_EXP);
