...
// Start midBattleEvolution
            ...
            if (IsMidBattleEvolutionOn())
            {
                if ((gLeveledUpInBattle & (1u << *expMonId)) && !(gTriedEvolving & (1u << *expMonId)))
                {
                    ...
                    u32 species = GetEvolutionTargetSpecies(&gPlayerParty[*expMonId], EVO_MODE_BATTLE_ONLY, gLeveledUpInBattle, NULL, &canStopEvo, DO_EVO);
    ...
    else
    {
        switch (ballId)
        {
            ...
            case BALL_LOVE:
                ...
                {
                    u8 gender1 = GetMonGender(GetBattlerMon(defId));
                    u8 gender2 = GetMonGender(GetBattlerMon(atkId));
