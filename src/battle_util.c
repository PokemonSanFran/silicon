...
bool32 TryStartingStatuses(void); // bdHazards
...
static u32 GetDeadMonsIndex(u32 battler)
{
    ...
    for (s32 i = 0; i < lastId; i++)
    {
        u32 species = GetMonData(&gParties[B_TRAINER_OPPONENT_A][i], MON_DATA_SPECIES_OR_EGG);
        if (species != SPECIES_NONE && species != SPECIES_EGG)
        {
            validMons++;
            if (gParties[B_TRAINER_OPPONENT_A][i].hp == 0)
                deadMons++;
        }
    }
