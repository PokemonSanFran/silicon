...
// Start fogBattle
static enum CancelerResult CancellerFog(struct BattleCalcValues *ctx)
{
    if (!IsFogBattle())
        return CANCELER_RESULT_SUCCESS;
    ...
    else
    {
        ...
        dmgCtx.battlerDef = ctx->battlerDef = gBattlerTarget;
        dmgCtx.move = dmgCtx.chosenMove = MOVE_NONE;
        dmgCtx.moveType = TYPE_MYSTERY;
        dmgCtx.isCrit = FALSE;
        dmgCtx.randomFactor = FALSE;
...
static enum MoveEndResult MoveEndAbsorb(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_STRENGTH_SAP:
        ...
        if ((gBattleStruct->passiveHpUpdate[cv->battlerAtk] > 0 && !IsBattlerUnaffectedByMove(cv->battlerDef) && IsBattlerAllowedToHeal(cv->battlerAtk)))
