...
//static u8 CalcBarFilledPixels(s32, s32, s32, s32 *, u8 *, u8); // monSummary
...
static void UpdateLvlInHealthbox(u8 healthboxSpriteId, u8 lvl)
{
    ...
    else
    {
        ...
        PrintUnknownLevel(lvl, battler, text);
        // End fogBattle
        UpdateIndicatorVisibilityAndType(healthboxSpriteId, TRUE);
    }
...
s32 MoveBattleBar(enum BattlerId battler, u8 healthboxSpriteId, u8 whichBar, u8 unused)
{
    ...
    // Start hpexpspeed
        ...
        u16 hpFraction = GetHPFraction(battler);
    // End hpexpspeed
    ...
    // Start hpexpspeed
        if (isBarInstant && whichBar == HEALTH_BAR)
            UpdateHpTextInHealthbox(gHealthboxSpriteIds[battler], HP_CURRENT, previousVal, gBattleSpritesDataPtr->battleBars[battler].maxValue);
    // End hpexpspeed
