...
void HandleSpeciesGfxDataChange(enum BattlerId battlerAtk, enum BattlerId battlerDef, u8 changeType)
{
    ...
    CreateBoxMon(&boxMon, targetSpecies, 5, personalityValue, OTID_STRUCT_PLAYER_ID);

    if (GetBattlerSide(battlerAtk) == B_SIDE_PLAYER)
    {
        ivs = GetMonData(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battlerAtk]], MON_DATA_IVS);
        GetMonData(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battlerAtk]], MON_DATA_NICKNAME, nickname);
        GetMonData(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battlerAtk]], MON_DATA_OT_NAME, otName);
    }
    else if (GetBattlerSide(battlerAtk) == B_SIDE_OPPONENT)
    {
        ivs = GetMonData(&gParties[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[battlerAtk]], MON_DATA_IVS);
        GetMonData(&gParties[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[battlerAtk]], MON_DATA_NICKNAME, nickname);
        GetMonData(&gParties[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[battlerAtk]], MON_DATA_OT_NAME, otName);
    }
